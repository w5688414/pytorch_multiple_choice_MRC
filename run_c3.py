...
class c3Processor(DataProcessor):
    def __init__(self, data_dir):
        ...
        for sid in range(3):
            data = []
            for subtask in ["d", "m"]:
                with open(self.data_dir + "/" + subtask + "-" + ["train.json", "dev.json", "test.json"][sid],"r", encoding="utf8") as f:
                    data += json.load(f)
            if sid == 0:
                random.shuffle(data)
            if(sid<2):
...
def main():
    ...
    if not args.do_train and not args.do_eval and not args.do_predict:
        raise ValueError("At least one of `do_train` or `do_eval` `do_predict`must be True.")
    ...
    if args.do_train:
        ...
        print('loading model')
        model_path=os.path.join(args.output_dir, "model_best_0.6768867924528302.pt")
        ...
        for ie in range(int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                if eval_accuracy >= best_accuracy:
                    torch.save(model.state_dict(), os.path.join(args.output_dir, "model_best_{}.pt".format(eval_accuracy)))
    ...
    print('loading model')
    model_path=os.path.join(args.output_dir, "model_best_0.6559224318658281.pt")
    print(model_path)
